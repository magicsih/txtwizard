<script lang="ts">
	...
	function minifyJS(js: string): string {
		// Basic, safe minification to avoid breaking code due to Automatic Semicolon Insertion.
		// This removes comments and unnecessary whitespace within lines, but preserves line breaks.
		return (
			js
				.replace(/\/\*[\s\S]*?\*\/|([^:]|^)\/\/.*/g, '$1') // Remove block and line comments
				.split('\n')
				.map((line) => line.trim())
